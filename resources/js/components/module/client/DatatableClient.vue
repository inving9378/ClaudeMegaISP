<template>
    ...
    <q-card>
      <q-table
        ...
        :selected-rows-label="
          (numberOfRows) =>
            `${numberOfRows} ${
              numberOfRows > 1 ? 'elementos seleccionados' : 'elemento seleccionado'
            }`
        "
        class="q-pa-sm"
      >
        ...
        <template #body="props">
          ...
          >
            <q-td style="width: 20px !important" v-if="selectedTable">
              <q-checkbox v-model="props.selected" dense :dark="darkMode" />
            </q-td>
            <template v-for="col in columns">
              <q-td :key="col.name" v-if="visibleColumns.includes(col.name)">
                <span v-if="col.name === 'status_smart'"
                  ><q-icon
                    :name="getOnuStatusIcon(props.row.status_smart)"
                    :color="getOnuStatusClass(props.row.status_smart)"
                    size="sm"
                  >
                    <q-tooltip>
                      {{ props.row.status_smart }}
                    </q-tooltip>
                  </q-icon>
                </span>
        ...
        <template v-slot:bottom-row>
          <q-tr>
            <q-td colspan="100%">
              <div>Nuevo - Cliente recien convertido sin servicios</div>
              <div>Activo - Clientes Activos</div>
              <div>Bloqueados - Clientes morosos que no han pagado sus servicios</div>
              <div>
                Inactivos - Clientes recurrentes, morosos, que se les vencio el periodo de
                gracia
              </div>
              <div>Cancelado - Cliente eliminado que hay esta en base de datos</div>
            </q-td>
          </q-tr>
        </template>
    ...
    >
      <template v-for="val in fieldsJson">
        <ComponentFormDefault
          v-if="val.include && val.field !== 'status_smart'"
          :json="val"
  ...
  <EditId :iDClient="iDClient" :module="module" @resetTable="resetTable"> </EditId>
</template>
...
import { exportTable, dataToFindExport } from "../../../helpers/ExportTablaExcel";
  ...
  setup(props, { emit }) {
    ...
    const setExpandFromColumn = () => {
      ...
      if (
        ...
        visibleColumns.value.length > getIndexFromColumn(expandAfterColumn.value)
      ) {
        expandedRows.value = cols.filter((c) => visibleColumns.value.includes(c.name));
