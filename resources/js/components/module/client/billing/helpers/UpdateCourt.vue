<template>
    <ModalCentrado
        id="modaleditUpdaeCourt"
        @submit="editCourtDate"
        @close-modal="cleanForm"
        modalTitle="Editar Fecha de Corte"
...
export default {
    ...
    setup(props, { emit }) {
        ...
        const editCourtDate = async () => {
            let url = `/cliente/edit_court_date`;
            let data = {
                id_client: props.iDClient,
                new_court_date: date.value,
            };
            try {
