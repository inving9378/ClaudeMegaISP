<template>
    <q-dialog
        ...
        maximized
    >
        <q-card class="import-csv-wizard">
            <q-toolbar class="bg-primary text-white">
                <q-toolbar-title>Importar CSV con previsualización</q-toolbar-title>
            ...
            <q-card-section class="q-pa-md" style="max-width: 1100px; margin: 0 auto; width: 100%">
                ...
                <div v-if="!previewData" class="text-center q-pa-xl">
                    <q-icon name="mdi-file-delimited-outline" size="64px" color="primary" />
                    <p class="q-mt-md">
                        Selecciona un archivo <strong>.csv</strong> con columnas de
                        latitud/longitud (y opcionalmente nombre/tipo/descripción — se detectan
                        automáticamente por el encabezado) para previsualizar los elementos antes
                        de importarlos al proyecto. Solo se importan puntos (nodos/POPs).
                    </p>
                    <q-file
                        v-model="file"
                        accept=".csv"
                    ...
                    <q-banner v-if="rowErrors" dense class="bg-negative text-white q-mt-md text-left">
                        <template v-slot:avatar>
                            <q-icon name="mdi-alert-circle-outline" />
                        </template>
                        El CSV tiene {{ rowErrors.length }} fila(s) con datos inválidos. Corrígelas
                        en el archivo y vuelve a intentar — no se importó nada.
                        <q-list dense class="q-mt-sm">
                            <q-item v-for="(err, idx) in rowErrors" :key="`row-err-${idx}`" class="q-pl-none">
                                <q-item-section>
                                    Fila {{ err.fila }}: {{ err.motivo }}
                                    <span v-if="err.valores">
                                        (lat: "{{ err.valores.lat }}", lng: "{{ err.valores.lng }}")
                                    </span>
                                </q-item-section>
                            </q-item>
                        </q-list>
...
const rowErrors = ref(null);
const loadingPreview = ref(false);
const loadingCommit = ref(false);

// MR-25 Fase 3b (#9990444) — solo entidades tipo marker (nodos/POPs), mismo vocabulario que
// ImportadorRedService::TIPOS_DETECTABLES.
...
const onPreview = async () => {
    ...
    errorMsg.value = null;
    rowErrors.value = null;
    const data = await previsualizarCsv(file.value);
...
const onConfirm = async () => {
    ...
    const reporte = await confirmarCsv(previewData.value.items, props.projectId ?? null);
