<template>
    <q-card class="my-card" flat bordered>
        <q-item>
            <q-item-section>
                <q-item-label>Ingresos extras</q-item-label>
            </q-item-section>
            <q-item-section class="no-padding" avatar>
                <form-extra-income-component
                    :box-id="box.id"
...
import FormExtraIncomeComponent from "./FormExtraIncomeComponent.vue";
...
const tableIdentifier = ref("billing-extra-income");
...
const hasAdd = computed(() => {
    return (
        props.hasPermission?.data.canView("seller_cuts_add_extra_icome") ??
        false
    );
});
...
const hasEdit = computed(() => {
    return (
        props.hasPermission?.data.canView("seller_cuts_edit_extra_icome") ??
        false
    );
});
...
const hasDelete = computed(() => {
    return (
        props.hasPermission?.data.canView("seller_cuts_delete_extra_icome") ??
        false
    );
});
...
const onRequest = async () => {
    loading.value = true;
    let data = await listExtraIncome(props.box.id);
    if (data !== null) {
        rows.value = data;
    } else {
        rows.value = [];
...
const destroy = async (object) => {
    Swal.fire({
        title: "Confirmación!",
        text: "Seguro que desea eliminar este ingreso extra?",
        icon: "question",
        ...
        if (result.isConfirmed) {
            object.loading = true;
            const result = await destroyExtraIncome(object.id);
            if (result) {
                message("Ingreso extra eliminado correctamente");
