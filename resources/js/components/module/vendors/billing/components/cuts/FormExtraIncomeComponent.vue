<template>
    ...
    <modal
        ...
        :title="`${object ? 'Editar' : 'Registrar'} ingreso extra`"
    >
        <template #body>
...
const store = async () => {
    loading.value = true;
    const result = await storeExtraIncome({
        ...formData.value,
        box_id: props.boxId,
    });
    if (result) {
        ...
        message("Ingreso extra adicionado correctamente");
    } else {
        error500();
    }
    loading.value = false;
...
const update = async () => {
    loading.value = true;
    const result = await updateExtraIncome(props.object.id, {
        ...formData.value,
    });
    if (result) {
        emits("updated", result);
        showModal.value = false;
        message("Ingreso extra modificado correctamente");
    } else {
        error500();
    }
    loading.value = false;
