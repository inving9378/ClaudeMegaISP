...
export default {
    ...
    setup(props, {emit}) {
        const id = ref(null);
        let submitButtonAction = props.action == '/administracion/package/add'
            ? "Crear Paquete"
            : "Salvar Paquete";
        ...
        const initComponent = async (action) => {
            ...
            if (action == '/administracion/package/add'){
                id.value = null;
                await getfieldsJson("Package");
            }else{
                id.value = partnerId;
                await getfieldsEdited("Package", partnerId);
            }
        };

        const getIdByAction = (action) => {
            return _.trimStart(action, '/administracion/package/update/')
        }
