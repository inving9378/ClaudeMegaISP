...
export default {
    ...
    setup(props, {emit}) {
        const id = ref(null);
        let submitButtonAction = props.action == '/administracion/ift/add'
            ? "Crear Ift"
            : "Salvar Ift";
        ...
        const initComponent = async (action) => {
            ...
            if (action == '/administracion/ift/add'){
                id.value = null;
                await getfieldsJson("Ift");
            }else{
                id.value = partnerId;
                await getfieldsEdited("Ift", partnerId);
            }
        };

        const getIdByAction = (action) => {
            return _.trimStart(action, '/administracion/ift/update/')
        }
