...
export default {
    ...
    setup(props, {emit}) {
        const id = ref(null);
        let submitButtonAction = props.action == '/administracion/document_type_template/add'
            ? "Crear Tipo de Plantilla"
            : "Salvar Tipo de Plantilla";
        ...
        const initComponent = async (action) => {
            ...
            if (action == '/administracion/document_type_template/add'){
                id.value = null;
                await getfieldsJson("DocumentTypeTemplate");
            }else{
                id.value = partnerId;
                await getfieldsEdited("DocumentTypeTemplate", partnerId);
            }
        };

        const getIdByAction = (action) => {
            return _.trimStart(action, '/administracion/document_type_template/update/')
        }
