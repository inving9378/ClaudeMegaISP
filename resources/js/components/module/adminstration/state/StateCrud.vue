...
export default {
    ...
    setup(props, {emit}) {
        const id = ref(null);
        let submitButtonAction = props.action == '/administracion/estado/add'
            ? "Crear Estado"
            : "Salvar Estado";
        ...
        const initComponent = async (action) => {
            ...
            if (action == '/administracion/estado/add'){
                id.value = null;
                await getfieldsJson("State");
            }else{
                id.value = partnerId;
                await getfieldsEdited("State", partnerId);
            }
        };

        const getIdByAction = (action) => {
            return _.trimStart(action, '/administracion/estado/update/')
        }
