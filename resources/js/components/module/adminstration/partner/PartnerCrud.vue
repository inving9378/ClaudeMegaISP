...
export default {
    ...
    setup(props, {emit}) {
        const id = ref(null);
        let submitButtonAction = props.action == '/administracion/socios/add'
            ? "Crear Socio"
            : "Salvar Socio";
        ...
        const initComponent = async (action) => {
            ...
            if (action == '/administracion/socios/add'){
                id.value = null;
                await getfieldsJson("Partner");
            }else{
                id.value = partnerId;
                await getfieldsEdited("Partner", partnerId);
            }
        };

        const getIdByAction = (action) => {
            return _.trimStart(action, '/administracion/socios/update/')
        }
