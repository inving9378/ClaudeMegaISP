...
export default {
    ...
    setup(props, {emit}) {
        const id = ref(null);
        let submitButtonAction = props.action == '/administracion/municipio/add'
            ? "Crear Municipio"
            : "Salvar Municipio";
        ...
        const initComponent = async (action) => {
            ...
            if (action == '/administracion/municipio/add'){
                id.value = null;
                await getfieldsJson("Municipality");
            }else{
                id.value = partnerId;
                await getfieldsEdited("Municipality", partnerId);
            }
        };

        const getIdByAction = (action) => {
            return _.trimStart(action, '/administracion/municipio/update/')
        }
