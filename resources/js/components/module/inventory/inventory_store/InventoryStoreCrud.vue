...
export default {
    ...
    setup(props, {emit}) {
        const id = ref(null);
        let submitButtonAction = props.action == '/inventory/inventory_store/add'
            ? "Crear Almacen"
            : "Salvar Almacen";
        ...
        const initComponent = async (action) => {
            ...
            if (action == '/inventory/inventory_store/add'){
                id.value = null;
                await getfieldsJson("InventoryStore");
            }else{
                id.value = partnerId;
                await getfieldsEdited("InventoryStore", partnerId);
            }
        };

        const getIdByAction = (action) => {
            return _.trimStart(action, '/inventory/inventory_store/update/')
        }
