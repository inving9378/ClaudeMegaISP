...
export default {
    name: "InventoryItemCustomModelCrud",
    props: {
        action: String,
    },
    components: {
    ...
    setup(props, { emit }) {
        ...
        let submitButtonAction =
            props.action == "/inventory/inventory_item_custom_model/add"
                ? "Crear Modelo de Articulo"
                : "Salvar Modelo de Articulo";
        ...
        const initComponent = async (action) => {
            ...
            if (action == "/inventory/inventory_item_custom_model/add") {
                id.value = null;
                await getfieldsJson("InventoryItemCustomModel");
            } else {
                id.value = partnerId;
                await getfieldsEdited("InventoryItemCustomModel", partnerId);
            }
        };
        ...
        const getIdByAction = (action) => {
            return _.trimStart(
                action,
                "/inventory/inventory_item_custom_model/update/"
            );
        };
