    ...
    <form method="POST" @submit.prevent="onSubmit" @change="dataForm.data.errors.clear($event.target.name)"
        ...
        <div class="modal-body m-0 row">
            <template v-for="val in fieldsJson">
                <ComponentFormDefault v-if="val.include" :id="id" :json="val" :errors="dataForm.data.errors" :key="val"
                    v-model="dataForm.data[val.field]" @update-field="updateThisField" @clear-error="clearError" />
            </template>

            <SelectStoreAndZone v-if="!isUpdate" :inventory_store_id="dataForm.data.inventory_store_id"
                :store_zone_id="dataForm.data.store_zone_id"></SelectStoreAndZone>
        </div>
...
export default {
    ...
    setup(props, { emit }) {
        ...
        let submitButtonAction =
            props.action == "/inventory/inventory_item/add-custom"
                ? "Crear Articulo"
                : "Salvar Articulo";
        ...
        const initComponent = async (action) => {    
            if (action == "/inventory/inventory_item/add-custom") {
                id.value = null;
                await getfieldsJson("InventoryItemCustom");
            } else {
                let inventoryItemId = getIdByAction(action);
                id.value = inventoryItemId;
                await getfieldsEdited("InventoryItemCustom", inventoryItemId);
        ...
        const onSubmit = () => {
            dataForm.data.inventory_item_custom_model_id = props.custom_model_id;
            dataForm.data
