...
import InventoryItemCustomCrud from "./InventoryItemCustomCrud.vue";
...
import InventoryMovementItemCustom from "./component/InventoryMovementItemCustom.vue";

export default {
    name: "InventoryItemCustomListar",
    components: {
        Datatable,
        InventoryItemCustomCrud,
        InventoryMovementItemCustom,
    ...
    setup(props) {
        const ns = `.leak983-inventoryItemCustomListar-${getCurrentInstance().uid}`;
        onUnmounted(() => {
            $(document).off(ns);
        });
        ...
        const action = ref("/inventory/inventory_item/add-custom");
