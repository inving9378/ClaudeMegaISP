...
export default {
    ...
    setup(props, { emit }) {
        ...
        let submitButtonAction =
            props.action == "/configuracion/template-task/add"
                ? "Crear Plantilla de Tarea"
                : "Salvar Plantilla de Tarea";
        ...
        const initComponent = async (action) => {
            ...
            if (action == "/configuracion/template-task/add") {
                id.value = null;
                await getfieldsJson("TemplateTask");
            } else {
                id.value = partnerId;
                await getfieldsEdited("TemplateTask", partnerId);
            }
        };
        ...
        const getIdByAction = (action) => {
            return _.trimStart(
                action,
                "/configuracion/template-task/update/"
            );
        };
