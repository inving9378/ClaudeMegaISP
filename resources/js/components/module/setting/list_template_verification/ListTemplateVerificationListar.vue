<template>
    <div class="d-flex flex-wrap gap-2 mb-2 justify-content-end">
        <button
            type="button"
            class="btn btn-outline-primary waves-effect waves-light ms-auto"
            @click="reload"
        >
            Refrescar
        </button>
        <button
            type="button"
            class="btn btn-outline-primary waves-effect waves-light"
            data-bs-toggle="modal"
            data-bs-target="#crudlisttemplateverification"
        >
            Agregar
        </button>
    </div>
    <Datatable
        module="configuracion/list-template-verification"
        model="ListTemplateVerification"
        list="Listado Plantillas de verificacion"
        @table="table"
        :editButton="{ modal: 'crudlisttemplateverification' }"
    ></Datatable>

    <div
        class="modal fade"
        id="crudlisttemplateverification"
        data-backdrop="static"
        data-keyboard="false"
    >
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h6 class="m-auto">{{ title }}</h6>
                </div>
                <ListTemplateVerificationCrud
                    :action="action"
                    :key="reloadCrud"
                    @close-modal="closeModal"
                ></ListTemplateVerificationCrud>
            </div>
        </div>
    </div>
</template>

<script>
import Datatable from "../../../base/shared/Datatable";
import { onMounted, reactive, ref } from "vue";
import DatatableHelper from "../../../../helpers/datatableHelper";
import ListTemplateVerificationCrud from "./ListTemplateVerificationCrud.vue";

export default {
    name: "ListTemplateVerificationListar",
    components: { Datatable, ListTemplateVerificationCrud },
    props: {
    },
    setup(props) {
        const title = ref("Crear Lista de Plantilla de Verificación");
        const datatable = reactive({
            table: new DatatableHelper({}),
        });
        const action = ref("/configuracion/list-template-verification/add");
        const reloadCrud = ref(true);

        onMounted(() => {
            $(document).on("click", ".uil-pen-modal", function () {
                let idItem = $(this).parent().attr("id-item");
                let modal = $(this).parent().attr("toggle-modal");
                showEditModal(idItem, modal);
            });
        });

        const closeModal = () => {
            $("#crudlisttemplateverification").modal("hide");
            reloadCrud.value = !reloadCrud.value;
            title.value = "Crear Lista de Plantilla de Verificación";
            action.value = "/configuracion/list-template-verification/add";
            datatable.table.reload();
        };

        const showEditModal = (idItem) => {
            $("#crudlisttemplateverification").modal("show");
            title.value = "Editar Lista de Plantilla de Verificación";
            action.value = `/configuracion/list-template-verification/update/${idItem}`;
        };

        const reload = () => {
            datatable.table.reload();
        };

        const table = (refTable) => {
            datatable.table = new DatatableHelper(refTable);
        };

        return {
            title,
            action,
            closeModal,
            table,
            reload,
            reloadCrud,
        };
    },
};
</script>

<style scoped></style>
