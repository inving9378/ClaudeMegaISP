<template>
    <div class="d-flex flex-wrap gap-2 mb-2">
        <button
            type="button"
            class="btn btn-outline-primary waves-effect waves-light ms-auto"
            @click="reload"
        >
            Refrescar
        </button>
        <button
            type="button"
            class="btn btn-outline-primary waves-effect waves-light"
            data-bs-toggle="modal"
            data-bs-target="#crudteam"
        >
            Agregar
        </button>
    </div>
    <Datatable
        module="configuracion/team"
        model="Team"
        list="Listado de Equipo"
        @table="table"
        :editButton="{ modal: 'crudteam' }"
        :select_filter="filters"
    ></Datatable>

    <div
        class="modal fade"
        id="crudteam"
        data-backdrop="static"
        data-keyboard="false"
    >
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h6 class="m-auto">{{ title }}</h6>
                </div>
                <TeamCrud
                    :action="action"
                    :key="reloadCrud"
                    @close-modal="closeModal"
                ></TeamCrud>
            </div>
        </div>
    </div>
</template>

<script>
import Datatable from "../../../base/shared/Datatable";
import { onMounted, reactive, ref } from "vue";
import DatatableHelper from "../../../../helpers/datatableHelper";
import TeamCrud from "./TeamCrud.vue";

export default {
    name: "TeamListar",
    components: { Datatable, TeamCrud },
    props: {
        filters: String,
    },
    setup(props) {
        const title = ref("Crear Equipo");
        const datatable = reactive({
            table: new DatatableHelper({}),
        });
        const action = ref("/configuracion/team/add");
        const reloadCrud = ref(true);

        onMounted(() => {
            $(document).on("click", ".uil-pen-modal", function () {
                let idItem = $(this).parent().attr("id-item");
                let modal = $(this).parent().attr("toggle-modal");
                showEditModal(idItem, modal);
            });
        });

        const closeModal = () => {
            $("#crudteam").modal("hide");
            reloadCrud.value = !reloadCrud.value;
            title.value = "Crear Equipo";
            action.value = "/configuracion/team/add";
            datatable.table.reload();
        };

        const showEditModal = (idItem) => {
            $("#crudteam").modal("show");
            title.value = "Editar Equipo";
            action.value = `/configuracion/team/update/${idItem}`;
        };

        const reload = () => {
            datatable.table.reload();
        };

        const table = (refTable) => {
            datatable.table = new DatatableHelper(refTable);
        };

        return {
            title,
            action,
            closeModal,
            table,
            reload,
            reloadCrud,
        };
    },
};
</script>

<style scoped></style>
