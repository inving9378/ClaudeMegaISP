...
export default {
    ...
    setup(props, {emit}) {
        const id = ref(null);
        let submitButtonAction = props.action == '/configuracion/work-flow/add'
            ? "Crear Flujo de Trabajo"
            : "Salvar Flujo de Trabajo";
        ...
        const initComponent = async (action) => {
            ...
            if (action == '/configuracion/work-flow/add'){
                id.value = null;
                await getfieldsJson("WorkFlow");
            }else{
                id.value = partnerId;
                await getfieldsEdited("WorkFlow", partnerId);
            }
        };

        const getIdByAction = (action) => {
            return _.trimStart(action, '/configuracion/work-flow/update/')
        }
