...
class TicketsDatatableHelper extends HelperDatatable
{
    private $model;
    private $columns;
    ...
    {
        return $field == 'estado' && $value == 'Nuevo';
    }

    public function hasFieldStatusAndValueCerrado($field, $value)
    {
        return $field == 'estado' && $value == 'Cerrado';
    }

    public function queryStatus($model, $values)
    {
        return $model->whereIn('estado', $values);
    }
    ...
    public function transform($request)
    {
        ...
        if (!empty($request['array'])) {
            foreach ($request['array'] as $key => $value) {
                ...
                $nestedData['action'] = view('meganet.shared.table.actions_without_permissions', [
                    'id' => $id,
                    'buttons' => $buttons,
                    'href' => '/tickets/ver/' . $id
                ])->toHtml();
