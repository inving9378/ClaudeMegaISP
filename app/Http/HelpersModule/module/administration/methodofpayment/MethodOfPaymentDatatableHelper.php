...
namespace App\Http\HelpersModule\module\administration\methodofpayment;
...
class MethodOfPaymentDatatableHelper extends HelperDatatable
{
    private $model, $columns;

    public function __construct()
    {
        $this->model = MethodOfPayment::class;
        $moduleName = 'MethodOfPayment';
        ...
        if (!empty($request['array'])) {
            foreach ($request['array'] as $key => $value) {
                ...
                $info = [
                    ...
                    'group' => 'methodofpayment',
                    'submodule' => 'methodofpayment'
                ];
