...
class ExpedienteController extends Controller
{
    ...
    /**
     * Acuse de avance en PDF con corte a fecha — evidencia para la mesa
     * directiva (item #9990551, sub-item de #9990531).
     *
     * Reusa exactamente lo que ya calcula `tablero()` (mismos apartados
     * visibles para ESTE usuario + `agregarGlobal()`), así el PDF nunca puede
     * divergir del tablero que el usuario ve en pantalla. Mismo gate que el
     * tablero (`documentacion-corporativa.view`, ya exigido por
     * `check_route_permission` a la entrada del módulo) — no crea un permiso
     * nuevo, siguiendo el criterio de que el tablero global no tiene un
     * permiso de exportación aparte del de sólo-lectura.
     *
     * `fecha_corte`: el módulo no guarda snapshots históricos (verificado —
     * todo el cálculo de completitud es en vivo), así que no se puede
     * reconstruir el estado exacto de una fecha pasada. Se acepta el
     * parámetro como ETIQUETA del acuse (para qué fecha se generó esta
     * evidencia) y, si es una fecha pasada, el PDF avisa explícitamente que
     * los valores son el estado actual — nunca se presenta un dato inventado
     * como si fuera histórico.
     */
    public function exportarAvance(Request $request)
    {
        $empresa = $this->empresas->actual();

        $fechaCorte = trim((string) $request->query('fecha_corte', ''));
        $corte = $fechaCorte === '' ? now()->startOfDay() : null;

        if ($corte === null) {
            try {
                $corte = \Illuminate\Support\Carbon::createFromFormat('Y-m-d', $fechaCorte)->startOfDay();
            } catch (\Throwable $e) {
                return response()->json([
                    'message' => "Fecha de corte inválida: '{$fechaCorte}'. Formato esperado: YYYY-MM-DD.",
                ], 422);
            }

            if ($corte->greaterThan(now()->startOfDay())) {
                return response()->json([
        ...
        $this->bitacora->exportar($empresa->id, null, null, [
            'pantalla'    => 'tablero.exportar',
            'fecha_corte' => $corte->toDateString(),
            'apartados_incluidos' => array_column($visibles, 'clave'),
        ]);

        $nombreBase = 'dc-acuse-avance-' . $corte->format('Ymd') . '-' . now()->format('Ymd-His') . '-' . Str::random(6);

        $pdf = Pdf::loadView('addon-documentacion-corporativa::export.avance', [
            'empresa'       => $empresa,
            'apartados'     => $visibles,
            'global'        => $global,
            'fechaCorte'    => $corte,
            'esCorteHoy'    => $corte->isToday(),
            'generado'      => now(),
        ]);
