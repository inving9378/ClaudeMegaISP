...
class TorreControlCatalog
{
    ...
    private static function gruposAutomatizacion(): array
    {
        return [
            ...
            self::control(
                ...
                'Con esto encendido nada avanza sin que tú lo autorices a mano, ni Jarvis ni el autopilot.',
                'verde',
            ),
            self::control(
                'circuito_revisor',
    ...
    // ── JARVIS ───────────────────────────────────────────────────────────────────────────────
    private static function gruposJarvis(): array
    {
        return [
            self::control(
                'jarvis.enabled',
                'config/circuito.php',
                self::fuenteConfig('circuito.jarvis.enabled'),
                'JarvisService',
                'Apaga a Jarvis entero.',
                'Sin Jarvis, ninguna terminal recibe respuesta al instante: todo lo que dude escala a requiere_irving.',
                'verde',
            ),
            self::control(
                'jarvis.mecanico.max_nivel',
                'config/circuito.php',
                self::fuenteConfig('circuito.jarvis.mecanico.max_nivel'),
                'JarvisService',
            ...
            self::control(
                'jarvis.mecanico.tope_diario',
                'config/circuito.php',
                self::fuenteConfig('circuito.jarvis.mecanico.tope_diario'),
                'JarvisService',
            ...
            self::control(
                'jarvis.ya_decidido.max_nivel',
                'config/circuito.php',
                self::fuenteConfig('circuito.jarvis.ya_decidido.max_nivel'),
                'TorreAutomationPolicy',
            ...
            self::control(
                'jarvis.exige_reversible_sin_recomendada',
                'config/circuito.php',
                self::fuenteConfig('circuito.jarvis.exige_reversible_sin_recomendada'),
                'JarvisService',
                'Si ninguna opción es reversible y no hay recomendada, escala.',
                'Apagarlo hace que Jarvis tome la primera opción sin más — más autonomía, más riesgo.',
                'verde',
            ),
            self::control(
                'jarvis.automerge.enabled',
                'config/circuito.php',
                self::fuenteConfig('circuito.jarvis.automerge.enabled'),
                'elegibleAutoMerge',
            ...
            self::control(
                'jarvis.automerge.cap_por_ciclo',
                'config/circuito.php',
                self::fuenteConfig('circuito.jarvis.automerge.cap_por_ciclo'),
                'DestrabarCommand',
            ...
            self::control(
                'jarvis.automerge.rutas_sensibles',
                'config/circuito.php',
                self::fuenteConfigCount('circuito.jarvis.automerge.rutas_sensibles', 'patrones'),
                'elegibleAutoMerge',
            ...
            self::control(
                'jarvis.automerge.patrones_destructivos',
                'config/circuito.php',
                self::fuenteConfigCount('circuito.jarvis.automerge.patrones_destructivos', 'patrones'),
                'elegibleAutoMerge',
            ...
            self::control(
                'jarvis.consolidado.enabled/horas_default/doc_path',
                'config/circuito.php',
                self::fuenteLiteral(null, 'ver config/circuito.php — jarvis.consolidado'),
                'JarvisService',
                'Consolidado estratégico: junta preguntas de negocio en una sola pregunta para Irving.',
                'horas_default=0 significa que Jarvis nunca procede solo en lo estratégico, siempre espera.',
                'verde',
            ),
            self::control(
                'jarvis.cierre.exige_reporte_coloquial',
                'config/circuito.php',
                self::fuenteConfig('circuito.jarvis.cierre.exige_reporte_coloquial'),
                'JarvisService',
            ...
            self::control(
                'jarvis.cierre.exige_enlace_revision',
                'config/circuito.php',
                self::fuenteConfig('circuito.jarvis.cierre.exige_enlace_revision'),
                'JarvisService',
    ...
    private static function gruposAutopilot(): array
    {
        return [
            self::control(
                ...
                'Sin autopilot, ningún brief se auto-decide: todo espera a Irving o a Jarvis.',
                'verde',
            ),
            self::control(
                'autopilot.max_nivel',
    ...
    private static function gruposRevisor(): array
    {
        return [
            ...
            self::control(
                ...
                'Es una SEGUNDA lista, distinta de la frontera dura de Jarvis. Cambiar un término cambia qué escala y qué no: eso es un diff revisado, no un textarea (fase 6 del plan, no aquí).',
                'azul',
            ),
            self::control(
                'revisor.perfil_path',
    ...
    private static function gruposDestrabador(): array
    {
        return [
            self::control(
                'jarvis.destrabe_bandeja.enabled',
                'config/circuito.php',
                self::fuenteConfig('circuito.jarvis.destrabe_bandeja.enabled'),
            ...
            self::control(
                'jarvis.destrabe_bandeja.intervalo_minutos',
                'config/circuito.php',
                self::fuenteConfig('circuito.jarvis.destrabe_bandeja.intervalo_minutos'),
                'tickDestrabe',
            ...
            self::control(
                'jarvis.destrabe_bandeja.limit',
                'config/circuito.php',
                self::fuenteConfig('circuito.jarvis.destrabe_bandeja.limit'),
                'tickDestrabe',
    ...
    private static function gruposTerminales(): array
    {
        return [
            ...
            self::control(
                'jarvis.cabida.umbral_segundos',
                'config/circuito.php',
                self::fuenteConfig('circuito.jarvis.cabida.umbral_segundos'),
                'circuito:cabida',
    ...
    private static function gruposFronteras(): array
    {
        ...
        return [
            self::control(
                ...
                'JarvisService::fronteraDuraDetalle → toda la cadena de aprobación',
            ...
            self::control(
                ...
                'JarvisService::fronteraDuraDeItemDetalle',
