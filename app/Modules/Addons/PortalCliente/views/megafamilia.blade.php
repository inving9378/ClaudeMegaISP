...
@push('scripts')
    ...
    <script>
    // La init del mapa de geocercas vive en el partial (window.mfGeoInit_<pid>):
    // soporta círculo y polígono. Aquí solo se carga Leaflet y se invoca al revelar
    // la sub-pestaña Geocercas.
    (function () {
        ...
        document.querySelectorAll('.mf-subtab2-btn').forEach(function (btn) {
            btn.addEventListener('click', function () {
                ...
                var gi = window['mfGeoInit_' + prof];
                if (sec.indexOf('geocercas-') === 0 && gi) gi();
            });
        });
