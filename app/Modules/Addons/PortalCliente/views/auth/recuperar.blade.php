...
<head>
    ...
    <title>Recuperar Contraseña — Portal Meganet</title>
    <style>
        :root { --pcolor: #0057a8; --pcolor-dark: #003d78; }
        * { box-sizing: border-box; margin: 0; padding: 0; }
        body { font-family: 'Inter', system-ui, sans-serif; background: linear-gradient(135deg, #0057a8 0%, #003d78 60%, #001f4d 100%); min-height: 100vh; display: flex; align-items: center; justify-content: center; padding: 1rem; }
        .auth-card { background: #fff; border-radius: 16px; padding: 2.5rem; width: 100%; max-width: 420px; box-shadow: 0 24px 64px rgba(0,0,0,.25); }
...
<div class="auth-card">
    ...
    <form method="POST" action="{{ route('portal.recuperar.post') }}">
        @csrf
        <div class="form-group">
            <label>Número de cliente</label>
            <input type="number" name="numero_cliente" value="{{ old('numero_cliente') }}" placeholder="Ej: 1234" min="1" required autofocus>
        </div>
        <div class="form-group">
            <label>Teléfono registrado</label>
            <input type="text" name="telefono" value="{{ old('telefono') }}" placeholder="Ej: 5512345678">
        </div>
        <div class="form-group">
