...
- Controller líneas: `.../Controllers/WhatsAppInstanceController.php` (`panel/index/store/getQr/connectionStatus/update/destroy` + Fase 3: `functionsCatalog/assignFunction/unassignFunction/reassignFunction`). ⚠️ `store()` llama `createInstance` en Evolution → NO usarlo para reflejar la instancia viva (duplicaría). `index()` eager-carga `functions`. `connectionStatus()` persiste `phone_number` (lazy, si vacío) leyendo `ownerJid` de Evolution.
...
- Service líneas: `.../Services/EvolutionApiService.php` (`getQrCode/getConnectionStatus/createInstance/sendAndLog` + `getInstanceProfile` → número real `ownerJid` sin `@s.whatsapp.net`).
- **Capa de funciones (Fase 3):** `Services/WhatsAppFunctionService.php` (assign/unassign/reassign/guardInstanceRemoval — reglas en backend, punto único), `Observers/WhatsAppInstance{,Function}Observer.php` (backstop, registrados en `ModuleServiceProvider::boot()`), `Exceptions/WhatsAppFunctionException.php` (render→422 español), `Services/WhatsAppLineResolver.php` (`lineForFunction(slug)` — lectura pura, **sin consumidores** hasta Fase 4c).
...
- **Fase 3 — HECHA (dev):** capa de funciones por línea. Tablas `whatsapp_functions` (catálogo, `exclusive` default true, seed Ventas/Cobranza/Soporte/Atención sin asignar) + `whatsapp_instance_functions`. Reglas en backend (`WhatsAppFunctionService` + observers): mover exclusiva, no dejar huérfana, reasignar antes de quitar, bloquear borrado de línea dueña única (verificadas por tinker). Permiso `whatsapp_manage_functions` (5 roles admin). UI: pantalla **Gestionar funciones** (`/whatsapp/funciones`, submenú revelado) + checks por línea en el panel con modales mover/reasignar/bloqueo. **Número real** en la tarjeta (`phone_number` persistido del `ownerJid`). `WhatsAppLineResolver` listo para Fase 4c (sin consumidores). **No toca el sender.** Commits `33f61641`→`624b0c9b`. Pendiente aparte (roadmap 196): Reiniciar/Desconectar reales.
