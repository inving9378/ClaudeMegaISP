...
## 2026-08-29 16:33 — Deriva de esquema #216: entregable final Fase 3 (tabla consolidada 3a+3b)

**wt-1, item #809 (Fase 3c del paraguas #216, vía #740).** Ensambla en una sola tabla los
resultados ya producidos por **#807** (Fase 3a, `storage/app/circuito/diff-esquema-216-fase3a.json`,
completado y mergeado a main en `685fcabb`) y **#808** (Fase 3b,
`storage/app/circuito/diff-esquema-216-fase3b.json`, 18 registros). Ambos archivos de salida
—gitignorados a propósito, son artefactos intermedios del circuito, no código— ya existían
completos en disco al arrancar esta vuelta.

**Nota sobre #808 y la condición de arranque del spec:** el spec de #809 exige "#807 y #808…deben
estar completado antes de arrancar". #807 cumple (estado `completado`). #808 quedó en
`requiere_irving` — no por rechazo de su contenido, sino porque una vuelta anterior lo dejó
correctamente resuelto (Irving aprobó las 3 preguntas del item el 2026-08-29 13:42, opción 1 en
las tres: "documentar, no ejecutar cambios"), produjo su JSON completo de 18 objetos a las 16:25,
pero la vuelta se cortó a los 600s sin poder correr el paso de `circuito:reportar`/marcar
`completado`, y el mecanismo de timeout re-escaló el item asumiendo "0 commits = 0 avance" — un
falso negativo porque la salida esperada de #808 es un JSON gitignorado, no un commit. Verificado
directamente el contenido de `diff-esquema-216-fase3b.json`: 18 objetos con el schema exacto que
pedía el spec (`tabla, elemento, tipo, falta_en, consumidores, desde_cuando, notas`), consistente
con las 15 diferencias `falta_en=megaisp` + 3 anomalías de migración descritas en la descripción
del item. No es un dato inventado — es la salida real y completa de un trabajo ya autorizado por
Irving. Se usa tal cual. **#808 no se tocó** (no es mi item — un item = un dueño); su cierre
formal en el roadmap queda pendiente para quien lo reclame, pero no bloquea este entregable porque
el dato que #809 necesita ya existe y es correcto.

**Drift de alcance heredado de #807:** el archivo de Fase 3a trae **54 registros**, no los 34
originales del item — entre el corte viejo (item #739, 49 hallazgos) y el archivo vigente,
`megaisp_dryrun` fue reconstruido (#797/#798) y el comando ganó comparación de índices (#800), así
que el diff actual tiene 131 registros con `lado_faltante=megaisp_dryrun`, de los cuales 54
corresponden a las tablas/columnas nombradas en la descripción original del item; los otros 77
(`auditoria_*`, `circuito_*`, `ipv6_*`, `torre_*`, `vigilante_*`, `dc_accesos_log`,
`dc_accionistas`, etc.) son diferencias nuevas fuera de alcance de #807/#809, documentadas ahí
como candidatas a un futuro barrido. Total real de este entregable: **72 registros (54+18)**, no
52 — la tabla de abajo trae los 72.

### Hallazgos más urgentes (consumidores reales — riesgo de bug tipo #209)

- **`referrals.referrals_referred_client_id_unique` ausente en la BD viva de dev** — el `UNIQUE`
  que impide referir dos veces al mismo cliente NO está enforced por el motor (0 duplicados hoy,
  pero nada lo impide si ocurre). 16+ consumidores reales (`ClientObserver`, `Referral`,
  `ProcessReferralCommissions`, Embajadores, Talento, PortalCliente, MegaFamilia).
- **`marketing_niches.preferred_voice_model`** — única columna de las 54 de Fase 3a con
  consumidores reales y activos (`VoiceComparatorController.php` +
  `VoiceComparatorView.vue`, comparador de voces TTS) pero **sin ninguna migración que la respalde**
  en el repo — probable `ALTER TABLE` manual durante el desarrollo. Candidata prioritaria a
  migración de reconciliación.
- **3 anomalías de migración** (Fase 3b): `jobs`/`failed_jobs` con migraciones **duplicadas**
  sobre las de Laravel base (`2021_11_01_113323_create_jobs_table.php` /
  `2019_08_19_000000_create_failed_jobs_table.php`), y un `dropIndex` en
  `2026_08_28_970000_add_review_flags_and_unique_client_invoice_id_to_invoices_table.php` sobre
  `invoices_client_invoice_id_index` que apunta a una migración predecesora no versionada en el
  repo. Las tres con consumidores reales (colas/facturación) — documentadas, sin tocar.

### Sin consumidores (18 de 72) — basura acumulada u omisión inofensiva, al final de la tabla
Tablas huérfanas genuinas (`client_cfdi_invoices`, `client_invoice_cfdi`,
`invoice_serviceables`), columnas de Flotas/roadmap sin código que las lea, y `users.password_legacy`
/ `password_migrated_at` (ni siquiera el propio `PasswordService` las usa). Candidatas a limpieza,
sin decisión de borrarlas tomada aquí.

⚠️ **Decisión explícita pendiente (fuera de alcance de este item, a propósito):** no se escribió
ninguna migración de reconciliación para ninguno de los 72 casos. Con esta tabla delante, Irving
decide caso por caso qué se documenta-y-deja, qué se resuelve con `ALTER`, y qué se borra.

**Total: 72 registros** (54 de Fase 3a + 18 de Fase 3b) — 54 con consumidores reales en el código (riesgo de bug tipo #209), 18 sin consumidores (basura acumulada u omisión inofensiva).

| tabla | columna/índice | lado en el que falta | quien la usa en el código | desde cuándo |
|---|---|---|---|---|
| api_integrations | api_integrations_slug_index | megaisp | app/Services/Core/ApiIntegrationService.php; app/Modules/Addons/Roadmap/Support/InventarioSemilla.php; app/Modules/Addons/Marketing/Services/Video/BrollLibraryService.php; app/Modules/Addons/Marketing/Services/Video/VideoTemplateRenderer.php; app/Modules/Addons/Empresa/Controllers/EmpresaManualController.php; app/Modules/Addons/MegaFamilia/Controllers/NotificacionesController.php; app/Modules/Addons/DocumentacionCorporativa/Controllers/ExpedienteController.php; app/Modules/Addons/DocumentacionCorporativa/Controllers/PlantillaController.php | 2026-05-29 00:30 (commit 50731e01) — misma migración que provider |
| dc_activos_digitales | (tabla completa) | migraciones | app/Modules/Addons/DocumentacionCorporativa/migrations/2026_08_29_140000_create_dc_activos_tables.php; app/Modules/Addons/DocumentacionCorporativa/Models/DcActivoDigital.php; app/Modules/Addons/DocumentacionCorporativa/Seeders/CatalogoSeeder.php; app/Modules/Addons/DocumentacionCorporativa/Controllers/RegistroEstructuradoController.php; app/Modules/Addons/DocumentacionCorporativa/Resolvers/InventarioResolver.php; resources/js/components/module/documentacion-corporativa/DcExpediente.vue; resources/js/components/module/documentacion-corporativa/DcRegistros.vue | 2026-08-28 22:23 (commit aea4fe47, "Fase 3 — tablas y modelos de activos, activos digitales e inventario de accesos") |
| dc_inventario_accesos | (tabla completa) | migraciones | app/Modules/Addons/DocumentacionCorporativa/migrations/2026_08_29_140000_create_dc_activos_tables.php; app/Modules/Addons/DocumentacionCorporativa/Models/DcInventarioAcceso.php; app/Modules/Addons/DocumentacionCorporativa/Seeders/CatalogoSeeder.php; app/Modules/Addons/DocumentacionCorporativa/Controllers/RegistroEstructuradoController.php; app/Modules/Addons/DocumentacionCorporativa/Resolvers/InventarioResolver.php; resources/js/components/module/documentacion-corporativa/DcExpediente.vue; resources/js/components/module/documentacion-corporativa/DcRegistros.vue | 2026-08-28 22:23 (commit aea4fe47, "Fase 3" de DocumentacionCorporativa) |
| dc_documentos | (tabla completa) | migraciones | app/Modules/Addons/DocumentacionCorporativa/migrations/2026_08_28_120100_create_dc_contenido_tables.php; app/Modules/Addons/DocumentacionCorporativa/Models/DcDocumento.php; app/Modules/Addons/DocumentacionCorporativa/Services/DocumentoService.php; app/Modules/Addons/DocumentacionCorporativa/Controllers/RegistroEstructuradoController.php; app/Modules/Addons/DocumentacionCorporativa/Resolvers/DocumentoResolver.php; app/Modules/Addons/DocumentacionCorporativa/Controllers/ConcesionController.php | 2026-08-28 16:11 (commit 18fff57b, circuito#662) |
| dc_solicitudes | (tabla completa) | migraciones | app/Modules/Addons/DocumentacionCorporativa/migrations/2026_08_29_150000_create_dc_solicitudes_table.php; app/Modules/Addons/DocumentacionCorporativa/Models/DcSolicitud.php; app/Modules/Addons/DocumentacionCorporativa/Controllers/DcSolicitudController.php; resources/js/components/module/documentacion-corporativa/DcExpediente.vue; app/Modules/Addons/DocumentacionCorporativa/Seeders/CatalogoSeeder.php; app/Modules/Addons/DocumentacionCorporativa/Resolvers/InventarioResolver.php | 2026-08-29 14:33 (commit 4615c1f5, circuito#760) |
| roadmap_items | agendado_para | migraciones | app/Modules/Addons/Roadmap/migrations/2026_08_20_190000_add_agendado_para_to_roadmap_items.php; app/Modules/Addons/Roadmap/Controllers/RoadmapController.php; app/Modules/Addons/Roadmap/Services/CompuertasService.php; app/Modules/Addons/Roadmap/Console/ReactivarAgendadosCommand.php; app/Modules/Addons/Roadmap/Controllers/TorreCompuertasController.php; app/Console/Kernel.php | ver migración app/Modules/Addons/Roadmap/migrations/2026_08_20_190000_add_agendado_para_to_roadmap_items.php |
| roadmap_items | automatizacion_override | migraciones | app/Modules/Addons/Roadmap/migrations/2026_08_19_100000_crea_torre_config_y_override_por_item.php; app/Modules/Addons/Roadmap/Services/TorreAutomationPolicy.php; app/Modules/Addons/Roadmap/Services/RoadmapCircuitoService.php; app/Modules/Addons/Roadmap/Controllers/RoadmapController.php; app/Modules/Addons/Roadmap/Support/TorreControlCatalog.php; resources/js/components/module/releases/torre-control/RoadmapTab.vue | ver migración app/Modules/Addons/Roadmap/migrations/2026_08_19_100000_crea_torre_config_y_override_por_item.php |
| roadmap_items | eta_metodo | migraciones | app/Modules/Addons/Roadmap/migrations/2026_08_09_172000_add_eta_trabajo_to_roadmap_items.php; app/Modules/Addons/Roadmap/Services/EstimadorTiempo.php; app/Modules/Addons/Roadmap/Services/RoadmapCircuitoService.php; app/Modules/Addons/Roadmap/Console/SchedulerCommand.php; app/Modules/Addons/Roadmap/Services/JarvisService.php; resources/js/components/module/releases/torre-control/TorreTerminales.vue | ver migración app/Modules/Addons/Roadmap/migrations/2026_08_09_172000_add_eta_trabajo_to_roadmap_items.php |
| roadmap_items | eta_segundos | migraciones | app/Modules/Addons/Roadmap/migrations/2026_08_09_172000_add_eta_trabajo_to_roadmap_items.php; app/Modules/Addons/Roadmap/Services/EstimadorTiempo.php; app/Modules/Addons/Roadmap/Services/JarvisService.php; app/Modules/Addons/Roadmap/Console/CabidaCommand.php; app/Modules/Addons/Roadmap/Services/RoadmapCircuitoService.php; app/Modules/Addons/Roadmap/Console/SchedulerCommand.php | ver migración app/Modules/Addons/Roadmap/migrations/2026_08_09_172000_add_eta_trabajo_to_roadmap_items.php |
| roadmap_items | frontera_valvula | migraciones | app/Modules/Addons/Roadmap/migrations/2026_08_20_170000_add_frontera_valvula_to_roadmap_items.php; app/Modules/Addons/Roadmap/Services/FronterasService.php; app/Modules/Addons/Roadmap/Controllers/RoadmapController.php; app/Modules/Addons/Roadmap/Services/JarvisService.php; resources/js/components/module/releases/torre-control/RoadmapTab.vue; resources/js/components/module/releases/torre-control/TorreControl.vue | ver migración app/Modules/Addons/Roadmap/migrations/2026_08_20_170000_add_frontera_valvula_to_roadmap_items.php |
| roadmap_items | origen_bloqueo | migraciones | app/Modules/Addons/Roadmap/migrations/2026_08_18_140000_add_origen_bloqueo_to_roadmap_items.php; app/Modules/Addons/Roadmap/Services/SupervisorService.php; app/Modules/Addons/Roadmap/Console/DestrabarCommand.php; app/Modules/Addons/Roadmap/Console/PriorizarSeguridadCommand.php; app/Modules/Addons/Roadmap/Console/RetriageFrenosCommand.php; app/Modules/Addons/Roadmap/Console/BackfillBloqueosCommand.php | ver migración app/Modules/Addons/Roadmap/migrations/2026_08_18_140000_add_origen_bloqueo_to_roadmap_items.php |
| dc_entregas | (tabla completa) | migraciones | app/Modules/Addons/DocumentacionCorporativa/migrations/2026_08_29_150100_create_dc_entregas_tables.php; app/Modules/Addons/DocumentacionCorporativa/Models/DcEntrega.php; app/Modules/Addons/DocumentacionCorporativa/Services/ActaEntregaService.php; app/Modules/Addons/DocumentacionCorporativa/Seeders/CatalogoSeeder.php; app/Modules/Addons/DocumentacionCorporativa/Resolvers/InventarioResolver.php | 2026-08-29 14:33 (commit 4615c1f5, circuito#760) |
| invoices | client_invoice_id | migraciones | database/migrations/2026_08_28_970000_add_review_flags_and_unique_client_invoice_id_to_invoices_table.php; app/Modules/Core/Clientes/Models/ClientInvoice.php; app/Modules/Core/Clientes/Repositories/ClientRepository.php; app/Console/Commands/Active/InvoicesReporteReconciliacionBackfillCommand.php; app/Console/Commands/Active/BackfillInvoicesFromClientInvoicesCommand.php | 2026-08-28 23:06 (commit bdcb8f7e, circuito#748, "migración aditiva needs_review/review_reason + UNIQUE client_invoice_id en invoices") |
| invoices | invoices_client_invoice_id_unique | migraciones | database/migrations/2026_08_28_970000_add_review_flags_and_unique_client_invoice_id_to_invoices_table.php; app/Modules/Core/Clientes/Models/ClientInvoice.php; app/Modules/Core/Clientes/Repositories/ClientRepository.php; app/Console/Commands/Active/InvoicesReporteReconciliacionBackfillCommand.php; app/Console/Commands/Active/BackfillInvoicesFromClientInvoicesCommand.php | 2026-08-28 23:06 (commit bdcb8f7e, circuito#748) |
| invoices | needs_review | migraciones | database/migrations/2026_08_28_970000_add_review_flags_and_unique_client_invoice_id_to_invoices_table.php; app/Modules/Core/Clientes/Models/ClientInvoice.php; app/Modules/Core/Clientes/Repositories/ClientRepository.php; app/Console/Commands/Active/InvoicesReporteReconciliacionBackfillCommand.php; app/Console/Commands/Active/BackfillInvoicesFromClientInvoicesCommand.php | 2026-08-28 23:06 (commit bdcb8f7e, circuito#748, "migración aditiva needs_review/review_reason + UNIQUE client_invoice_id en invoices") |
| invoices | review_reason | migraciones | database/migrations/2026_08_28_970000_add_review_flags_and_unique_client_invoice_id_to_invoices_table.php; app/Modules/Core/Clientes/Models/ClientInvoice.php; app/Modules/Core/Clientes/Repositories/ClientRepository.php; app/Console/Commands/Active/InvoicesReporteReconciliacionBackfillCommand.php; app/Console/Commands/Active/BackfillInvoicesFromClientInvoicesCommand.php | 2026-08-28 23:06 (commit bdcb8f7e, circuito#748, "migración aditiva needs_review/review_reason + UNIQUE client_invoice_id en invoices") |
| referrals | idx_referral_chain | megaisp | app/Observers/ClientObserver.php; app/Models/Referrals/Referral.php; app/Modules/Addons/Embajadores/Controllers/ClientesController.php (where chain_path like %/{id}/%); app/Modules/Addons/MegaFamilia/Controllers/ApiController.php; app/Console/Commands/Active/RebuildReferralClosuresCommand.php | 2026-05-29 00:35 (commit 2439b74a) — database/migrations/2026_05_26_300006_create_referrals_table.php declara $table->index('chain_path', 'idx_referral_chain') |
| roadmap_items | eta_minutos | migraciones | app/Modules/Addons/Roadmap/migrations/2026_08_25_170000_add_eta_thomas_to_roadmap_items.php; app/Modules/Addons/Roadmap/Services/JarvisService.php; app/Modules/Addons/Roadmap/Controllers/RoadmapController.php; app/Modules/Addons/Roadmap/Services/RoadmapCircuitoService.php; resources/js/components/module/releases/torre-control/RoadmapTab.vue | ver migración app/Modules/Addons/Roadmap/migrations/2026_08_25_170000_add_eta_thomas_to_roadmap_items.php |
| roadmap_items | trabajo_iniciado_at | migraciones | app/Modules/Addons/Roadmap/migrations/2026_08_09_172000_add_eta_trabajo_to_roadmap_items.php; app/Modules/Addons/Roadmap/Services/EstimadorTiempo.php; app/Modules/Addons/Roadmap/Services/RoadmapCircuitoService.php; app/Modules/Addons/Roadmap/Console/SchedulerCommand.php; resources/js/components/module/releases/torre-control/TorreTerminales.vue | ver migración app/Modules/Addons/Roadmap/migrations/2026_08_09_172000_add_eta_trabajo_to_roadmap_items.php |
| api_integrations | api_integrations_provider_index | megaisp | app/Models/Core/ApiIntegration.php; app/Modules/Addons/Hub/Controllers/ApiIntegrationController.php; app/Modules/Addons/Payments/migrations/2026_06_30_140400_seed_native_payment_provider.php; app/Modules/Core/Release/Controllers/AuditController.php | 2026-05-29 00:30 (commit 50731e01, "feat(core): fase 4.5a — Integration Hub centro único de API keys") — database/migrations/2026_05_27_500001_create_api_integrations_table.php declara ->index() en la columna provider desde su creación |
| invoices | invoices_client_invoice_id_index (dropIndex en database/migrations/2026_08_28_970000_add_review_flags_and_unique_client_invoice_id_to_invoices_table.php) | migracion_fantasma_predecesora_no_versionada | app/Modules/Core/Clientes/Models/ClientInvoice.php; app/Modules/Core/Clientes/Repositories/ClientRepository.php; app/Console/Commands/Active/InvoicesReporteReconciliacionBackfillCommand.php; app/Console/Commands/Active/BackfillInvoicesFromClientInvoicesCommand.php | database/migrations/2026_08_28_970000_... existe en git desde 2026-08-28 23:06 (commit bdcb8f7e, circuito#748). Su dropIndex(['client_invoice_id']) asume que ya existe un índice simple sobre esa columna, creado por database/migrations/2026_08_28_192630_add_client_invoice_id_to_invoices_table.php — ese archivo SÍ está registrado como 'Ran' en la tabla migrations de esta BD (batch 576), pero NO EXISTE en el árbol de git (git ls-files falla, y el archivo tampoco está en disco). |
| marketing_messages | marketing_messages_external_message_id_index | megaisp | app/Modules/Addons/Marketing/Jobs/UpdateMessageStatusJob.php; app/Modules/Addons/Marketing/Jobs/ProcessIncomingMessageJob.php; app/Modules/Addons/Marketing/Jobs/SendOutboundMessageJob.php; app/Modules/Addons/Marketing/Jobs/DownloadWhatsAppMediaJob.php | 2026-05-26 13:59 (commit e5aa44a9) — database/migrations/2026_05_26_100017_create_marketing_messages_table.php |
| referrals | idx_referral_status | megaisp | app/Jobs/Referrals/ApplyReferralCommissions.php (where status='approved'); app/Jobs/Referrals/CalculateDailyStats.php (where status='applied'); app/Modules/Addons/Embajadores/Controllers/ComisionesController.php (where status=$status); app/Modules/Addons/Embajadores/Controllers/DashboardController.php (where status!='cancelled', where status='pending') | 2026-05-29 00:35 (commit 2439b74a) |
| referrals | referrals_referred_client_id_unique | megaisp | app/Observers/ClientObserver.php; app/Models/Referrals/Referral.php; app/Jobs/Referrals/ProcessReferralCommissions.php; 13 archivos más referencian referred_client_id (Embajadores, Talento, PortalCliente, MegaFamilia) | 2026-05-29 00:35 (commit 2439b74a) — la migración declara ->unique() sobre referred_client_id |
| roadmap_items | eta_asignada_at | migraciones | app/Modules/Addons/Roadmap/migrations/2026_08_25_170000_add_eta_thomas_to_roadmap_items.php; app/Modules/Addons/Roadmap/Services/JarvisService.php; app/Modules/Addons/Roadmap/Controllers/RoadmapController.php; resources/js/components/module/releases/torre-control/RoadmapTab.vue | ver migración app/Modules/Addons/Roadmap/migrations/2026_08_25_170000_add_eta_thomas_to_roadmap_items.php |
| roadmap_items | reabre_item_id | migraciones | app/Modules/Addons/Roadmap/migrations/2026_08_29_090000_add_reabre_item_id_to_roadmap_items.php; app/Modules/Addons/Roadmap/ModuleServiceProvider.php; app/Modules/Addons/Roadmap/Services/ReconstruccionP0Service.php; app/Modules/Addons/Roadmap/Console/ReconstruirP0Command.php | ver migración app/Modules/Addons/Roadmap/migrations/2026_08_29_090000_add_reabre_item_id_to_roadmap_items.php |
| roadmap_items | reanudaciones_timeout | migraciones | app/Modules/Addons/Roadmap/migrations/2026_08_20_150000_add_reanudacion_timeout_to_roadmap_items.php; app/Modules/Addons/Roadmap/Services/JarvisService.php; app/Modules/Addons/Roadmap/Console/ParquearTimeoutCommand.php; app/Modules/Addons/Roadmap/Controllers/RoadmapController.php | ver migración app/Modules/Addons/Roadmap/migrations/2026_08_20_150000_add_reanudacion_timeout_to_roadmap_items.php |
| dc_documento_versiones | (tabla completa) | migraciones | app/Modules/Addons/DocumentacionCorporativa/migrations/2026_08_28_120100_create_dc_contenido_tables.php; app/Modules/Addons/DocumentacionCorporativa/Models/DcDocumentoVersion.php; app/Modules/Addons/DocumentacionCorporativa/Services/DocumentoService.php | 2026-08-28 16:11 (commit 18fff57b, circuito#662, "migraciones aditivas de catálogo, contenido y bitácora") |
| dc_entrega_items | (tabla completa) | migraciones | app/Modules/Addons/DocumentacionCorporativa/migrations/2026_08_29_150100_create_dc_entregas_tables.php; app/Modules/Addons/DocumentacionCorporativa/Models/DcEntregaItem.php; app/Modules/Addons/DocumentacionCorporativa/Models/DcEntrega.php | 2026-08-29 14:33 (commit 4615c1f5, circuito#760) — migración app/Modules/Addons/DocumentacionCorporativa/migrations/2026_08_29_150100_create_dc_entregas_tables.php |
| marketing_messages | marketing_messages_sent_at_index | megaisp | app/Modules/Addons/Marketing/Services/PromptBuilder.php (orderBy sent_at desc); app/Models/Marketing/Conversation.php (hasMany Message orderBy sent_at); app/Modules/Addons/Marketing/Controllers/MarketingConversationController.php (orderByDesc sent_at) | 2026-05-26 13:59 (commit e5aa44a9) |
| referral_prospects | referral_prospects_converted_client_id_foreign | megaisp | app/Observers/ClientObserver.php; app/Models/Referrals/ReferralProspect.php; app/Modules/Addons/PortalCliente/Controllers/EmbajadoresController.php | 2026-05-29 00:35 (commit 2439b74a, "feat(embajadores): migraciones tablas referral_* y client_referral_profiles") — database/migrations/2026_05_26_300004_create_referral_prospects_table.php |
| referral_prospects | referral_prospects_converted_client_id_foreign | megaisp | app/Observers/ClientObserver.php; app/Models/Referrals/ReferralProspect.php; app/Modules/Addons/PortalCliente/Controllers/EmbajadoresController.php | 2026-05-29 00:35 (commit 2439b74a) |
| roadmap_items | estado_previo_claim | migraciones | app/Modules/Addons/Roadmap/migrations/2026_08_20_150000_add_reanudacion_timeout_to_roadmap_items.php; app/Modules/Addons/Roadmap/Services/RoadmapCircuitoService.php; app/Modules/Addons/Roadmap/Console/ParquearTimeoutCommand.php | ver migración app/Modules/Addons/Roadmap/migrations/2026_08_20_150000_add_reanudacion_timeout_to_roadmap_items.php |
| roadmap_items | sin_ui | migraciones | app/Modules/Addons/Roadmap/migrations/2026_08_21_220000_add_sin_ui_to_roadmap_items.php; app/Modules/Addons/Roadmap/Services/JarvisService.php; app/Modules/Addons/Roadmap/Controllers/RoadmapController.php | ver migración app/Modules/Addons/Roadmap/migrations/2026_08_21_220000_add_sin_ui_to_roadmap_items.php |
| roadmap_items | veces_timeouteo | migraciones | app/Modules/Addons/Roadmap/migrations/2026_08_28_220000_add_veces_timeouteo_to_roadmap_items.php; app/Modules/Addons/Roadmap/Console/ParquearTimeoutCommand.php; app/Modules/Addons/Roadmap/Services/JarvisService.php | ver migración app/Modules/Addons/Roadmap/migrations/2026_08_28_220000_add_veces_timeouteo_to_roadmap_items.php |
| marketing_generated_content | marketing_generated_content_company_id_index | megaisp | app/Modules/Addons/Marketing/Controllers/MarketingGeneratedContentController.php; app/Modules/Addons/Marketing/routes.php | 2026-05-26 13:59 (commit e5aa44a9, "feat(marketing): fases 1 y 2 — fundación BD + captura de leads") — database/migrations/2026_05_26_100014_create_marketing_generated_content_table.php |
| marketing_niches | preferred_voice_model | migraciones | app/Modules/Addons/Marketing/Controllers/VoiceComparatorController.php; resources/js/components/module/marketing/VoiceComparatorView.vue | desconocido — sin migración en ningún módulo, a diferencia de preferred_voice_id (columna hermana) que sí la tiene |
| roadmap_items | bloqueo_expira_en | migraciones | app/Modules/Addons/Roadmap/migrations/2026_08_18_140000_add_origen_bloqueo_to_roadmap_items.php; app/Modules/Addons/Roadmap/Console/RetriageFrenosCommand.php | ver migración app/Modules/Addons/Roadmap/migrations/2026_08_18_140000_add_origen_bloqueo_to_roadmap_items.php |
| roadmap_items | bloqueo_renovaciones | migraciones | app/Modules/Addons/Roadmap/migrations/2026_08_18_140000_add_origen_bloqueo_to_roadmap_items.php; app/Modules/Addons/Roadmap/Console/RetriageFrenosCommand.php | ver migración app/Modules/Addons/Roadmap/migrations/2026_08_18_140000_add_origen_bloqueo_to_roadmap_items.php |
| roadmap_items | frontera_valvula_at | migraciones | app/Modules/Addons/Roadmap/migrations/2026_08_20_170000_add_frontera_valvula_to_roadmap_items.php; app/Modules/Addons/Roadmap/Controllers/RoadmapController.php | ver migración app/Modules/Addons/Roadmap/migrations/2026_08_20_170000_add_frontera_valvula_to_roadmap_items.php |
| roadmap_items | huecos_medidos_at | migraciones | app/Modules/Addons/Roadmap/migrations/2026_08_25_180000_add_huecos_spec_to_roadmap_items.php; app/Modules/Addons/Roadmap/Services/RevisorService.php | ver migración app/Modules/Addons/Roadmap/migrations/2026_08_25_180000_add_huecos_spec_to_roadmap_items.php |
| roadmap_items | huecos_spec | migraciones | app/Modules/Addons/Roadmap/migrations/2026_08_25_180000_add_huecos_spec_to_roadmap_items.php; app/Modules/Addons/Roadmap/Services/RevisorService.php | ver migración app/Modules/Addons/Roadmap/migrations/2026_08_25_180000_add_huecos_spec_to_roadmap_items.php |
| roadmap_items | sin_ui_motivo | migraciones | app/Modules/Addons/Roadmap/migrations/2026_08_21_220000_add_sin_ui_to_roadmap_items.php; app/Modules/Addons/Roadmap/Services/JarvisService.php | ver migración app/Modules/Addons/Roadmap/migrations/2026_08_21_220000_add_sin_ui_to_roadmap_items.php |
| failed_jobs | 2026_05_27_140743_create_failed_jobs_table.php (duplicado sobre 2019_08_19_000000_create_failed_jobs_table.php de Laravel) | duplicado_en_tabla_migrations | Illuminate\Queue (registro de jobs fallidos: artisan queue:failed, queue:retry) | El propio Laravel ya trae 2019_08_19_000000_create_failed_jobs_table (batch 1). La migración custom 2026_05_27_140743_create_failed_jobs_table (batch 379, mismo commit 941b3162 que la anomalía de jobs, corrida justo un batch después) es un duplicado exacto SIN guard Schema::hasTable('failed_jobs'). |
| jobs | 2026_05_26_143203_create_jobs_table.php + 2026_05_27_140742_create_jobs_table.php (duplicado sobre 2021_11_01_113323_create_jobs_table.php de Laravel) | duplicado_en_tabla_migrations | Illuminate\Queue (toda la infraestructura de colas: dispatch(), artisan queue:work, Supervisor workers de Marketing/CobranzaBlaster/video-render) | El propio Laravel ya trae 2021_11_01_113323_create_jobs_table (batch 1) — la que efectivamente creó la tabla al origen del proyecto. Encima existen 2 migraciones CUSTOM redundantes, ambas registradas como 'Ran' en la tabla migrations: 2026_05_26_143203_create_jobs_table (batch 363, commit 6cd165b6 'snapshot cambios locales movidos desde main', SÍ tiene guard Schema::hasTable('jobs') → corrió como no-op) y 2026_05_27_140742_create_jobs_table (batch 378, commit 941b3162 'feat(marketing): fase 3 — agente IA + WhatsApp via Evolution API', SIN guard). |
| marketing_generated_content | marketing_generated_content_template_id_index | megaisp | app/Modules/Addons/Marketing/Controllers/MarketingGeneratedContentController.php | 2026-05-26 13:59 (commit e5aa44a9) |
| marketing_messages | marketing_messages_sender_user_id_index | megaisp | app/Models/Marketing/Message.php (relación belongsTo User::class via sender_user_id) | 2026-05-26 13:59 (commit e5aa44a9) |
| referrals | referrals_prospect_id_foreign | megaisp | app/Models/Referrals/Referral.php (belongsTo ReferralProspect::class via prospect_id) | 2026-05-29 00:35 (commit 2439b74a) |
| referrals | referrals_prospect_id_foreign | megaisp | app/Models/Referrals/Referral.php | 2026-05-29 00:35 (commit 2439b74a) |
| roadmap_items | roadmap_items_automatizacion_override_index | migraciones | app/Modules/Addons/Roadmap/migrations/2026_08_19_100000_crea_torre_config_y_override_por_item.php | ver migración app/Modules/Addons/Roadmap/migrations/2026_08_19_100000_crea_torre_config_y_override_por_item.php |
| roadmap_items | roadmap_items_bloqueo_expira_en_index | migraciones | app/Modules/Addons/Roadmap/migrations/2026_08_18_140000_add_origen_bloqueo_to_roadmap_items.php | ver migración app/Modules/Addons/Roadmap/migrations/2026_08_18_140000_add_origen_bloqueo_to_roadmap_items.php |
| roadmap_items | roadmap_items_frontera_valvula_index | migraciones | app/Modules/Addons/Roadmap/migrations/2026_08_20_170000_add_frontera_valvula_to_roadmap_items.php | ver migración app/Modules/Addons/Roadmap/migrations/2026_08_20_170000_add_frontera_valvula_to_roadmap_items.php |
| roadmap_items | roadmap_items_origen_bloqueo_index | migraciones | app/Modules/Addons/Roadmap/migrations/2026_08_18_140000_add_origen_bloqueo_to_roadmap_items.php | ver migración app/Modules/Addons/Roadmap/migrations/2026_08_18_140000_add_origen_bloqueo_to_roadmap_items.php |
| roadmap_items | roadmap_items_reabre_item_id_index | migraciones | app/Modules/Addons/Roadmap/migrations/2026_08_29_090000_add_reabre_item_id_to_roadmap_items.php | ver migración app/Modules/Addons/Roadmap/migrations/2026_08_29_090000_add_reabre_item_id_to_roadmap_items.php |
| client_cfdi_invoices | (tabla completa) | migraciones | — (sin consumidores) | desconocido — sin migración en database/migrations/, migrations_old/ ni ningún módulo addon; cero referencias en app/ resources/ routes/ config/ |
| client_invoice_cfdi | (tabla completa) | migraciones | — (sin consumidores) | desconocido — sin migración en database/migrations/, migrations_old/ ni ningún módulo addon; cero referencias en app/ resources/ routes/ config/ |
| fleet_assignments | deleted_at | migraciones | — (sin consumidores) | desconocido — la migración base (app/Modules/Addons/Flotas/migrations/2026_06_01_180000_create_fleet_tables.php, commit 29885cbe, 2026-06-02) crea esta tabla SIN softDeletes() (sí lo hace para fleet_vehicles/fleet_providers/fleet_maintenances/fleet_documents en el mismo archivo) |
| fleet_fuel_log | deleted_at | migraciones | — (sin consumidores) | desconocido — la migración base (app/Modules/Addons/Flotas/migrations/2026_06_01_180000_create_fleet_tables.php, commit 29885cbe, 2026-06-02) crea esta tabla SIN softDeletes() (sí lo hace para fleet_vehicles/fleet_providers/fleet_maintenances/fleet_documents en el mismo archivo) |
| fleet_photos | deleted_at | migraciones | — (sin consumidores) | desconocido — la migración base (app/Modules/Addons/Flotas/migrations/2026_06_01_180000_create_fleet_tables.php, commit 29885cbe, 2026-06-02) crea esta tabla SIN softDeletes() (sí lo hace para fleet_vehicles/fleet_providers/fleet_maintenances/fleet_documents en el mismo archivo) |
| fleet_subscriptions | base_price | migraciones | — (sin consumidores) | desconocido — no aparece en app/Modules/Addons/Flotas/migrations/2026_06_02_120000_create_fleet_subscriptions_tables.php ni en ninguna otra migración del repo |
| fleet_subscriptions | included_units | migraciones | — (sin consumidores) | desconocido — no aparece en app/Modules/Addons/Flotas/migrations/2026_06_02_120000_create_fleet_subscriptions_tables.php ni en ninguna otra migración del repo |
| fleet_subscriptions | overage_amount | migraciones | — (sin consumidores) | desconocido — no aparece en app/Modules/Addons/Flotas/migrations/2026_06_02_120000_create_fleet_subscriptions_tables.php ni en ninguna otra migración del repo |
| fleet_subscriptions | overage_unit_price | migraciones | — (sin consumidores) | desconocido — no aparece en app/Modules/Addons/Flotas/migrations/2026_06_02_120000_create_fleet_subscriptions_tables.php ni en ninguna otra migración del repo |
| fleet_subscriptions | overage_units | migraciones | — (sin consumidores) | desconocido — no aparece en app/Modules/Addons/Flotas/migrations/2026_06_02_120000_create_fleet_subscriptions_tables.php ni en ninguna otra migración del repo |
| invoice_serviceables | (tabla completa) | migraciones | — (sin consumidores) | desconocido — sin migración en database/migrations/, migrations_old/ ni ningún módulo addon; cero referencias en app/ resources/ routes/ config/ |
| marketing_generated_content | marketing_generated_content_source_plan_id_index | megaisp | — (sin consumidores) | 2026-05-26 13:59 (commit e5aa44a9) |
| roadmap_items | preguntas_natural | migraciones | — (sin consumidores) | desconocido — sin migración en ningún módulo; no está en $fillable de RoadmapItem.php ni se referencia en ningún archivo del repo |
| roadmap_items | preguntas_natural_at | migraciones | — (sin consumidores) | desconocido — sin migración en ningún módulo; no está en $fillable de RoadmapItem.php ni se referencia en ningún archivo del repo |
| roadmap_items | resumen_natural | migraciones | — (sin consumidores) | desconocido — sin migración en ningún módulo; no está en $fillable de RoadmapItem.php ni se referencia en ningún archivo del repo |
| roadmap_items | resumen_natural_at | migraciones | — (sin consumidores) | desconocido — sin migración en ningún módulo; no está en $fillable de RoadmapItem.php ni se referencia en ningún archivo del repo |
