...
| **SEG-TRIAGE** | `circuito:priorizar-seguridad` (Opus). **2A.3:** disparo por EVENTO (`ClasificarRiesgoJob` al crear el item) + barrido diario de respaldo — antes cron cada 3 min | backlog con señales regex | `priority=alta`, `requiere_irving`, tags `[SEG-TOP]`/`[DINERO-TOP]`, y **`origen_bloqueo='clasificador'` + `motivo_bloqueo`** para negocio/prod | qué es riesgo real vs ruido | **frenar el despacho** (es ADVISORY: informa, no detiene — sólo `origen_bloqueo='humano'` frena); auto-aprobar nada sensible | sello `⟪SEG-TRIAGE⟫` + columna |
