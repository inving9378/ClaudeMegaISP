...
| **Paso 3 (pieza 1/4) — Usuario de sistema "Asistente IA" (trazabilidad)** | Crea la IDENTIDAD del bot para que sus acciones futuras (aplicar pagos) queden atribuidas a su nombre igual que Diana/Tere. NO construye la IA ni la aplicación de pagos aún. Migración aditiva `users += is_system` (boolean, default false) + migración idempotente (por email) del usuario `Asistente IA` (`asistente.ia@sistema.local`, id 4843 en DEV), `is_system=true`, `color` cian distintivo (`rgba(0,184,217,1)`) para el historial. **Login IMPOSIBLE:** password inusable (bcrypt de secreto aleatorio) + `estado='inactivo'` (`LoginController::attemptLogin` exige `estado==='activo'`; `estado` es `enum('activo','bloqueado','inactivo')`, sin valor 'sistema'). `login_user` es NOT NULL en el esquema → se le puso valor, pero los dos candados anteriores bloquean. **SIN rol/permisos** (0 roles, 0 permisos): solo identidad. Resolver fiable: `User::SYSTEM_AI_EMAIL` + `User::systemAi()`. El historial muestra "Asistente IA" (autor vía `payments.add_by`→`getClientNameWithFathersNamesAttribute`). Commit `6f73e544`. Verificado en DEV (existe/is_system/color; no loguea; systemAi() resuelve; 0 permisos). | ✅ Resuelto | — |
| **Paso 3 — PENDIENTE (pieza de cableado): reapuntar la autoría de pagos automáticos al usuario IA** | Hoy `PaymentApplicationService::resolveSystemUserId()` busca el rol `SUPER_ADMIN` (que NO existe; los reales son `super-administrator`/`Mostrador`/`DESARROLLADOR`) → cae al **fallback `user id 1` = "Admin"**, así que los pagos del webhook SPEI/OpenPay se ven como "Admin". En la pieza donde la IA aplique pagos: apuntar `add_by` a `User::systemAi()` y darle SOLO el permiso de aplicar pagos. NO se tocó en la pieza 1. | ⏳ Pendiente | Media |
