...
| **Render de error en UI de saldos de vendedores (#109 deuda futura)** | Cuando se conecten `getCurrentBalanceAccount` / `getIncomesAccount` (`resources/js/components/module/vendors/billing/helper/helper.js`) a una UI, el consumidor debe renderizar el `null` como estado **"Error / no disponible"**, NO como `$0`. El helper ya devuelve `null` defensivamente (#109 Paso 1: antes tragaba el error y mostraba `$0`, cero falso engañoso para decisiones de dinero); falta el lado de la **vista** cuando exista el componente que lo consuma. | ⚠️ Pendiente | Baja |
